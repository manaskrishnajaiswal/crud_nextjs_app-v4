...
// PUT /api/modelApi/modelsData/[modeName]/[modelData] -> update individual model data to db
export async function putDBData(req, res) {
  console.log(req.query);
  ...
  try {
    ...
    if (modelNames.includes(modelName)) {
      ...
      const modDBCustomSchema = await getSchemaForModel(schemaFromStoredFile);
      const MyModelFetch = mongoose.model(modelName, modDBCustomSchema);
      const customData = await MyModelFetch.findById(modelDataId);
      if (!customData) {
        return res.status(404).json({ message: "DB Data not found" });
      }
      const updatedData = {};
      Object.keys(modelData).forEach((key) => {
        if (moment(modelData[key], "YYYY-MM-DD", true).isValid()) {
          updatedData[key] = new Date(modelData[key]) || customData[key];
        } else {
          updatedData[key] = modelData[key] || customData[key];
        }
      });
      const updatedCustomData = await MyModelFetch.findByIdAndUpdate(
        modelDataId,
        updatedData,
        { new: true }
      );
      res.status(200).json({
        message: `Model: ${modelName} found in database! and data updated successfully`,
        model: modelName,
        result: updatedCustomData,
        found: true,
      });
    } else {
      res.status(404).json({
